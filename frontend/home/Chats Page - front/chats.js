...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch("https://komodo-hub.onrender.com/api/list_chats/", {
        method: "POST",
        headers: {
    ...
    .then(data => {
        ...
        data.forEach((user, index) => {
            ...
            chat.className = "chat";
            chat.innerHTML = `
                <img src="${user.image}" alt="Profile image" class="profile_image">
                <div class="button-text">${user.name}</div>
            `;

            chat.addEventListener("click", function(){
                if (user.conversation_id) {
                    window.location.href = `chat_content.html?conversation_id=${user.conversation_id}`;
                } else {
                    console.error("Missing conversation ID in API response:", user);
